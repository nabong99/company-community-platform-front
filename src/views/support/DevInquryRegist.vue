	...
	<div id="contentWrap">
		...
		<div id="Content">
			...
			<div class="ListTbWrap mt-20">
				<!-- <template v-if="devInquryData.secretAt"> -->
            ...
            <div class="btn_wrap">	
				<template v-if="this.$route.query.status != 'C'">
					<a href="#" class="btn04" @click="getUpdate()">수 정</a>
					<a href="#" class="btn03" @click="goList()">취 소</a>
				</template>
				<template v-else>
					<a href="#" class="btn04" @click="doRegist()">등 록</a>
					<a href="#" class="btn03" @click="goList()">취 소</a>
				</template>
			</div>
...
<script>
import { devInquryRegist, devInquryView,devInquryUpdate } from "@/api/devInquryApi";
	...
	methods : {
		...
		getView() {
			devInquryView(
				...
				this.$route.query.inqrySn
			).then(response => {
				console.log("response.data.devInquryView", response.data);
				this.devInquryData = response.data.vo;
		...
		doRegist() {
			...
			if (!confirm("Q&A를 등록 하시겠습니까?")) return false;

			devInquryRegist(this.devInquryData
			).then(response => {
				console.log("response.data.devInquryRegist", response.data.resultSn);
				alert("Q&A가 등록 되었습니다.");
				//alert("inqury >>"+response.data.vo.resultSn);
				
				this.goView(response.data.resultSn);
			}).catch(error => {
				alert("Q&A  등록이 실패하였습니다.\r\n관리자에게 문의하여주시기 바랍니다.");
				this.goList();
			});
		},
		//글 수정
		getUpdate(){
			this.devInquryData.inqrySn =  this.$route.query.inqrySn;
			
			if (!confirm("Q&A를 수정 하시겠습니까?")) return false;

			devInquryUpdate(this.devInquryData.inqrySn,this.devInquryData)
			.then(response => {
				console.log("response.data.getUpdate", response.data.resultSn);
				alert("Q&A가 수정 되었습니다.");
				//alert("inqury >>"+response.data.vo.resultSn);
				
				this.goView(response.data.resultSn);
			}).catch(error => {
				alert("Q&A 수정이 실패하였습니다.\r\n관리자에게 문의하여주시기 바랍니다.");
				this.goList();
			});
		},
		doFileChange(e) {
